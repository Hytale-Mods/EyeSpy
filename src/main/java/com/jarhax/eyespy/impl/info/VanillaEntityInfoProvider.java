...
public class VanillaEntityInfoProvider implements InfoProvider<EntityContext> {
    ...
    public void provideInfo(EntityContext context, InfoBuilder info) {
        ...
        // Check for NPC Entity using entityRef
        final NPCEntity npcInfo = context.entityStore().getComponent(context.entityRef(), Objects.requireNonNull(NPCEntity.getComponentType()));
        if (npcInfo != null) {
            // Owner Info
